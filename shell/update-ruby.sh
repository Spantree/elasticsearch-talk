...
function check_ruby_symlinks() {
    ...
    if [ ! -L '/usr/bin/ruby' ] || [ ! -L '/usr/bin/gem' ]; then
        echo "Ruby or Gem not symlinked"
        rm '/var/puppet-init/install-ruby'
    ...
    if [ "grep '/usr/local/rvm/wrappers/default' ${RUBY_SYMLINK}" ]; then
        echo "Ruby symlink is old-style"
        rm '/usr/bin/ruby'
...
if [[ -f '/var/puppet-init/install-ruby' ]]; then
    exit 0
fi
...
touch '/var/puppet-init/install-ruby'
