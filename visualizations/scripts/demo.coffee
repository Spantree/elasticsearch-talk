class Aggregation
	...
	constructor: (opts) ->
		...
		if opts.dimensionFunction
			@dimensionFunction = opts.dimensionFunction
		if opts.reduceSumFunction
			@reduceSumFunction = opts.reduceSumFunction
		if opts.chartPostSetup
			@chartPostSetup = opts.chartPostSetup
...
flattenAggregationResponse = (aggregations, results, parentData) ->
	...
	for bucket in buckets
		...
		if(childAggregations.length == 0)
			...
			children = flattenAggregationResponse(childAggregations, bucket, obj)
...
window.myController = ($scope) ->
	...
	genderAgg = new TermsAggregation
		name: 'gender',
		field: 'gender'
	...
	ageAgg = new HistogramAggregation
		name: 'age'
		field: 'age'

	durationAgg = new HistogramAggregation
		name: 'trip_duration'
		field: 'trip_duration'
		interval: 60
		chartPostSetup: (c) ->
	...
	.then (resp) ->
		data = flattenAggregationResponse(aggregations, resp.aggregations)
